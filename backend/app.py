...
class QueryResponse(BaseModel):
    ...
    answer: str
    sources: List[Union[str, SourceInfo]]  # Support both string and SourceInfo
    session_id: str
...
async def query_documents(request: QueryRequest):
    ...
    try:
        ...
        # Convert sources to SourceInfo objects if they're dicts
        formatted_sources = []
...
async def clear_session(session_id: Optional[str] = None):
    """Clear a specific session or create a new one"""
    try:
        if session_id:
            rag_system.session_manager.clear_session(session_id)
        # Always return a new session ID for the frontend
